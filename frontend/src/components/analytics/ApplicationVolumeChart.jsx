...
//
// Props:
//   data — array of opportunity objects from the backend:
//          [{ opportunityTitle, count, status, location, opportunityId, statusBreakdown }]
//
// The chart now also renders a status breakdown tooltip using the
// statusBreakdown field that the backend provides.
...
export default function ApplicationVolumeChart({ data = [] }) {
  ...
  const options = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          // Show total + status breakdown from backend data
          label: (ctx) => {
            const opp = data[ctx.dataIndex];
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Live from{" "}
            <code className="bg-gray-100 px-1.5 py-0.5 rounded text-[#035b9d] font-mono text-[11px]">
