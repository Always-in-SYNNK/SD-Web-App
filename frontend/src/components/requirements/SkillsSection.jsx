...
const SkillsSection = ({ skills, onChange }) => {
  ...
  return (
    <section className="bg-white p-6 rounded-lg">
      ...
      {skills.map((skill, i) => (
        <article key={i} className="p-2 border rounded mb-2 flex justify-between items-center">
          <strong>{skill}</strong>
          <button onClick={() => removeSkill(i)} className="text-red-400 hover:text-red-600 text-sm">Remove</button>
        </article>
      ))}
