...
export default function AdminSection({ isAdmin, source = "applicant" }) {
  ...
  if (loading) {
    return (
      <section className="h-12 rounded-lg bg-gray-100 animate-pulse" aria-label="Loading admin section" />
    );
  }
  ...
  return (
    <section className="space-y-2">
      ...
      {errorMessage && (
        <p className="text-[11px] text-red-500 px-1" role="alert">{errorMessage}</p>
      )}
    </section>
  );
}